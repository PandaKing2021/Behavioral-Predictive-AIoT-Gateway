...
// Include card reader required libraries
#include <SPI.h>
#include <MFRC522.h>
#include <Wire.h>
...
// Note: Door security device ID must contain "security" in its name
...
// Card identification main function
void cardLogic(){
  ...
  if ( ! mfrc522.PICC_ReadCardSerial()) {
    Serial.println("No card to select");
    return;
  }
  ...
  if(cardUid == permittedUid){
    permitStatus = 1;
    buzzerStart(100);
    oled_string_display(2,42,30,"Allowed",1); // Display access status
  }else{
    permitStatus = 0;
    buzzerStart(100);
    delay(100);
    buzzerStart(100);
    oled_string_display(2,42,30,"Denied ",1); // Display access status
  }
