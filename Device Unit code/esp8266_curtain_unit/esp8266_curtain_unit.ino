...
// Include configuration file (please copy config_template.h to config.h and modify settings first)
#include "config.h"

// Include OLED required libraries
#include <Adafruit_GFX.h>
#include <Adafruit_SSD1306.h>

// Include BH1750 light intensity sensor (I2C communication) required library
#include <Wire.h>
#include <BH1750.h>
...
// Initialize light intensity sensor object
BH1750 lightMeter(BH1750_ADDR);

// Sensor data initialization
int cur_light = 0; // LED light status
int curtain_status = 0;  // Curtain open/close status
float cur_brightness = 0.0;  // Light intensity

int access_status = 0; // Monitor door access status

int recv_light = 0; // Control LED light data
int recv_curtain_status = 0;

int per_curtain_status = 0; // Used to store previous curtain status
...
void loop() {
  // Get light intensity sensor data
  getBrightness();
...
// Gateway send/receive processing section
void sendMsgToGate(){
...
void getMsgFromGate(){
  if(client.available()){   
    StaticJsonDocument<200> msg;
    String jsonStr = client.readStringUntil('\n'); // Get data, carriage return as end
...
void controlDevice(){
  if(recv_curtain_status == 1 && per_curtain_status != 1){ // 1 = open command
    Serial.println("Open.");
    ...
    digitalWrite(CURTAIN_PIN1,LOW);
    digitalWrite(CURTAIN_PIN2,LOW);
  }else if(recv_curtain_status == 0 && per_curtain_status != 0){ // 0 = close command
...
void oled_string_display(int textsize,int oled_x,int oled_y,char* str,int if_clear){
  if(if_clear == 1)
  oled.setTextColor(WHITE, BLACK);
  oled.setTextSize(textsize);// Set font size  
...
/*
else if(recv_curtain_status == 0 && per_curtain_status != 0){ // 0 = standby command
