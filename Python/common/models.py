"""Gateway Runtime Shared State Model.

Uses threading.Lock to protect all shared states, replacing global variables in the original code.
"""
...
class GatewayState:
    """Gateway runtime shared state, all fields are thread-safe through threading.Lock protection."""
    ...
    def __init__(self) -> None:
        ...
        # Whether device node has started data collection
        self._source_start_flag: int = 0
        # Event to replace listen_if_sensor_connected busy waiting
    ...
    def is_device_permitted(self, device_id: str) -> bool:
        """Check if device is in the permitted list."""
        with self._lock:
    ...
    def wait_for_sensor(self, timeout: float = None) -> bool:
        """Block until device node starts collecting data.

        Args:
            timeout: Timeout in seconds, None means wait indefinitely.

        Returns:
            True if device connected, False if timed out.
        """
