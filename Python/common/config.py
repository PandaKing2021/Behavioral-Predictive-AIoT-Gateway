...
Provides functions for reading, validating, and loading configuration files, using dataclass to define configuration structures.
Configuration file format remains compatible with the original project (line-by-line reading, no section headers).
"""
...
class AliyunIotConfig:
    """Alibaba Cloud IoT connection configuration."""
    product_key: str = ""
    device_name: str = ""
    device_secret: str = ""
    region_id: str = ""
...
@dataclass
class GateConfig:
    """Gateway complete configuration, aggregating all sub-configurations."""
...
def _read_config_lines(filepath: Path) -> list[str]:
    """Read non-empty lines from configuration file, stripping trailing newlines.
...
def load_gate_config(config_dir: Optional[Path] = None) -> GateConfig:
    ...
    Args:
        config_dir: Directory containing configuration file, defaults to current directory.
    ...
    if len(lines) < 8:
        raise ValueError(f"GateConfig.txt insufficient configuration items, expected 8 lines, actual {len(lines)} lines")
...
def load_user_config(config_dir: Optional[Path] = None) -> UserConfig:
    ...
    if len(lines) < 3:
        raise ValueError(f"UserConfig.txt insufficient configuration items, expected 3 lines, actual {len(lines)} lines")
...
def write_user_config(config: UserConfig, config_dir: Optional[Path] = None) -> None:
    ...
    logger.info("User configuration written: %s", filepath)
...
def load_server_config(config_dir: Optional[Path] = None) -> ServerNetworkConfig:
    ...
    if len(lines) < 2:
        raise ValueError(f"serverConfig.txt insufficient configuration items, expected 2 lines, actual {len(lines)} lines")
