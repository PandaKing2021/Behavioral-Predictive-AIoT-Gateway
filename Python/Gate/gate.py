"""IoT Smart Gateway Main Entry.

Initializes configuration, database, shared state, and starts communication module threads.
"""
...
def check_user_config_with_server(db_socket: socket.socket, user_config: UserConfig) -> None:
    ...
    If local configuration is illegally modified, attempt automatic correction.
...
def fetch_permitted_devices(db_socket: socket.socket, device_key: str) -> list:
    """Fetch permitted device list from database server.
...
def main():
    ...
    # Initialize AI prediction modules
    try:
        sensor_handler.init_prediction_modules(gate_db_conn)
        logger.info("AI prediction modules initialized")
    except Exception as error:
        logger.warning("AI prediction modules initialization failed: %s, will use threshold decision mode", error)
