...
Adds test mode, making database server connection optional.
"""
...
# Add project root and Gate directory to sys.path to ensure correct module imports
...
def connect_db_server(config: GateConfig) -> socket.socket:
    ...
    Returns:
        Database server TCP socket, or None (test mode).
    ...
    try:
        ...
        logger.info("Database server connection successful: %s:%d",
                     config.db_server.ip, config.db_server.db_server_port)
        return db_socket
    except OSError as error:
        logger.error("Database server connection failed: %s", error)
        raise
...
def check_user_config_with_server(db_socket: socket.socket, user_config: UserConfig) -> None:
    ...
    Args:
        db_socket: Database server TCP socket (can be None in test mode).
        user_config: Local user configuration.
    """
    if TEST_MODE or db_socket is None:
        logger.warning("Test mode: skipping user configuration validation")
        return
    ...
    if status_code == 1:
        ...
        logger.warning("Local user configuration is invalid, checking for correction...")
        try:
...
def fetch_permitted_devices(db_socket: socket.socket, device_key: str) -> list:
    """Fetch permitted device list from database server (returns default list in test mode).
    ...
    if status_code == 1:
        ...
        logger.info("Successfully fetched permitted device info: %s", devices)
        return devices
    else:
        logger.error("Failed to fetch permitted device info: %s", device_list)
        return []
...
def main():
    ...
    if TEST_MODE:
        ...
        print("   - Skipping user configuration validation")
        print("="*60)
    ...
    try:
        ...
        if not TEST_MODE:
            ...
            logger.warning("Test mode: continuing, skipping local database")
    ...
    try:
        ...
        if TEST_MODE:
            logger.warning("Test mode: database server connection failed, continuing: " + str(e))
        else:
    ...
    if db_socket is None:
        logger.warning("Test mode: Android handler will not be able to communicate with database server")
