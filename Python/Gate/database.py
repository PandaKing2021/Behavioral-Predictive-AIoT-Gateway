"""Gateway Local Database Operations Module.
...
def create_database_connection(db_config: GateDbConfig, database: str = None) -> MySQLConnection:
    ...
    Args:
        ...
        database: Database name, if None no database is specified.
...
def init_gate_database(db_config: GateDbConfig) -> MySQLConnection:
    """Initialize gateway local database, create database and table if not exist.
    ...
    Returns:
        MySQLConnection to gate_database.

    Raises:
        mysql.connector.Error: Database initialization failed.
    """
    # First connection: create database and table
    ...
    try:
        ...
        conn.commit()
        logger.info("Gateway local database and table initialized")
    finally:
        conn.close()
...
def save_sensor_data(conn: MySQLConnection, data: dict) -> None:
    ...
    Args:
        ...
        data: Sensor data dict, must contain the following keys:
