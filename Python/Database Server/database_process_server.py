...
Responsible for handling gateway database operation requests, including user registration,
user configuration validation, device queries, etc. Uses MySQL database for persistent
storage and communicates with the gateway via TCP Socket. All communication uses JSON format uniformly.
"""
...
class DatabaseServer:
    ...
    def _client_handler(self, cs: socket.socket) -> None:
        ...
        try:
            ...
            logger.warning("Gateway %s connection disconnected: %s", cs.getpeername(), error)
        except (json.JSONDecodeError, ValueError) as error:
            logger.error("Failed to parse gateway request data: %s", error)
        except Exception as error:
            logger.error("Gateway request processing exception: %s", error)
        finally:
            cs.close()
    ...
    def _add_new_user(self, cs: socket.socket, data_code) -> None:
        ...
        Args:
            cs: Gateway TCP socket.
            data_code: User info dict (containing username, password, device_key).
        """
        ...
        try:
            ...
            if insert_status != 0:
                ...
                logger.warning("New user addition failed: possible primary key or unique key conflict")
    ...
    def _check_userconfig_illegal(self, cs: socket.socket, data_code) -> None:
        """Validate gateway local user configuration.
        ...
        try:
            ...
            if result:
                ...
                logger.warning("Gateway user configuration is invalid: %s", username)
                send_json(cs, format_comm_data_string(send_op, "NULL", 0))
                ...
                if result:
                    logger.info("Detected illegally modified configuration, correcting user '%s'", username)
            ...
            logger.error("User configuration validation exception: %s", error)
            send_json(cs, format_comm_data_string(send_op, str(error), 2))
        finally:
            cursor.close()

    def _check_device_id(self, cs: socket.socket, data_code) -> None:
        """Query device name list by device key.
